...
import com.chtrembl.petstore.product.control.ProductRepository;
import com.chtrembl.petstore.product.control.mapping.ProductMapper;
        ...
        if (acceptType != null && acceptType.contains("application/json")) {
            ...
            try {
                var products = this.productRepository.findAll().stream()
                        .map(ProductMapper.INSTANCE::toProductModel)
                        .toList();

                var productsJSON = this.objectMapper.writeValueAsString(products);
