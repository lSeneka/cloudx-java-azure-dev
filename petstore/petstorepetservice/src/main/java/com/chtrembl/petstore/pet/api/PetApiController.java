...
import com.chtrembl.petstore.pet.control.PetRepository;
import com.chtrembl.petstore.pet.control.mapping.PetMapper;
    ...
    public ResponseEntity<List<Pet>> findPetsByStatus(
        ...
        if (acceptType != null && acceptType.contains("application/json")) {
            PetApiController.log.info(String.format(
                    ...
                    status));
            try {
                var pets = this.petRepository.findAll().stream()
